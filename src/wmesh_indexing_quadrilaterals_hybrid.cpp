					    ...
					    wmesh_int_p lquadrilateral_idx_)
{
 ...
 wmesh_status_t wmesh_indexing_quadrilaterals_hybrid_hash(wmesh_int_t 		cell_type_,
						     wmesh_int_t 		c2n_m_,
						     wmesh_int_t 		c2n_n_,
...
wmesh_status_t wmesh_indexing_quadrilaterals_hybrid_calculate(wmesh_int_t 				cell_type_,
...
extern "C"
{
  wmesh_status_t  wmesh_indexing_quadrilaterals_hybrid(wmesh_int_t		ntypes_,
						  ...
						  wmesh_int_p		work_)
  {
	...
	if (c2n_n_[i]>0)
	  {
	    wmesh_status_t status = wmesh_indexing_quadrilaterals_hybrid_hash(i,
									 c2n_m_[i],
									 c2n_n_[i],
									 c2n_v_ + c2n_ptr_[i],
									 c2n_ld_[i],
	...
	if (c2q_n_[k]>0)
	  {
	    for (int j=c2q_n_[k]-1;j>=0;--j)
	      {
		for (int i=c2q_m_[k]-1;i>=0;--i)
		  {
		    std::cout << "quadrilateral ("<< k << "," << j << "," << i << ")" <<  std::endl;
		    auto s = c2q_v_[c2q_ptr_[k]+j*c2q_ld_[k]+i];		    
		    static constexpr const wmesh_int_t d = - std::numeric_limits<wmesh_int_t>::max();
		    while (s < 0 && s != d)
		      {
			auto ltype = GenericEncoding<wmesh_int_t,2>::Low(-s);
			auto lidx = (GenericEncoding<wmesh_int_t,2>::Up(-s)-1)/c2q_ld_[ltype];
			auto llidx = (GenericEncoding<wmesh_int_t,2>::Up(-s)-1)%c2q_ld_[ltype];
			std::cout << "          linked quadrilateral ("<< ltype << "," << lidx << "," << llidx << ")" <<  std::endl;
			s = c2q_v_[c2q_ptr_[ltype]+lidx*c2q_ld_[ltype]+llidx];		    
		      }		    
		  }
	      }
	  }
    ...
    for (int i=0;i<4;++i)
      {
	wmesh_status_t status = wmesh_indexing_quadrilaterals_hybrid_calculate(i,
									  c2n_ptr_,
									  c2n_m_,
									  c2n_n_,
									  c2n_v_,
