...
template<typename T>
struct wmesh_template_integral_diffusion_t
{
  struct data_t
  {    
    ...
    data_t(const wmesh_template_integral_diffusion_t<T>&  parent_);
...
template<typename T>
struct wmesh_integral_diffusion_t : public wmesh_integral_t< wmesh_integral_diffusion_t<T> , T>
{
  ...
  {    
    ...
    wmesh_mat_t<T>	m_q_jacobians_det;
    wmesh_mat_t<T> 	m_build_rhs;
    data_t(const wmesh_integral_diffusion_t<ELEMENT,T>&  parent_);
  };
