...
wmesh_template_integral_flux_t<T>::data_t::data_t(const wmesh_template_integral_flux_t<T>&  parent_)
{
  ...
  wmesh_mat_t<T>::alloc(&this->m_local_matrix,ndofs_trial,ndofs_test);
...
{
  const wmesh_int_t facet 		= data_.m_facet;
  const wmesh_int_t facet 		= data_.m_facet_type;
  const wmesh_int_t ielement_type 	= data_.m_ielement_type;
  const wmesh_int_t ifacet_idx 		= data_.m_ifacet_idx;
  const wmesh_int_t ifacet_rotation 	= data_.m_ifacet_rotation;
  const wmesh_int_t jelement_type 	= data_.m_jelement_type;
  const wmesh_int_t jfacet_idx 		= data_.m_jfacet_idx;
  const wmesh_int_t jfacet_rotation 	= data_.m_jfacet_rotation;
